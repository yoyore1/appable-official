/*--------------------------------------------------------------------------------------
 *  Design palette research — ported from web `designResearch.ts`.
 *--------------------------------------------------------------------------------------*/

import type { InterviewTurn, MasterBuildPrompt, Vibe } from './appableBuilderTypes.js';
...
export function isDeferToRecommendation(answer: string): boolean {
	...
	if (!a) { return true; }
	return /recommend|you think|u think|think is best|pick for me|your call|up to you|whatever you|you decide|surprise|idk|don't know|anything|best.*color|what.*best/i.test(a);
}

export function recommendDesignProfile(ctx: string, vibeOverride?: Vibe): DesignProfile {
	...
	if (/recipe|food|cook|meal|kitchen|dish|grocery|ingredient/.test(c)) {
		...
		return {
			colors: moms ? 'Sage green, warm cream, soft terracotta accents' : 'Warm terracotta, cream white, forest green accents',
	...
	if (/dog|pet|walk|walker|paw|puppy/.test(c)) {
		return {
			...
			rationale: 'Green and cream feel friendly and outdoorsy — pet apps should feel trustworthy and warm, not clinical.',
...
function interviewBlob(interview: InterviewTurn[]): string {
	const a = (id: string) => interview.find((t) => t.questionId === id)?.answer ?? '';
	return [a('idea'), a('twist'), a('reference_name'), a('audience'), a('features')].filter(Boolean).join(' ');
}
