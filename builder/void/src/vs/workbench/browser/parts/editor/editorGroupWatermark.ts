...
export class EditorGroupWatermark extends Disposable {
	...
	constructor(
		...
		// Appable Builder: hide Void's cube/letterpress entirely (chat-first product surface).
	...
	private render(): void {
		...
		// Appable Builder: the editor area is the app preview surface, not a Void splash.
		// Show a warm, on-brand placeholder instead of Void's cube + keybindings.
		const box = append(this.shortcuts, $('.appable-watermark'));

		const title = append(box, $('.appable-watermark-title'));
		title.textContent = 'Your app preview will appear here';

		const sub = append(box, $('.appable-watermark-sub'));
		sub.textContent = 'Hit “Let’s build it” and watch it come to life.';
	}
